...
Imported['MotionSetting'] = 1.01;

/*:
 * @plugindesc ver1.00/スキルの行動モーションを再設定します。
 * @author Yana
 *
...
(function(){
    ...
    Game_Actor.prototype.setWeaponImageId = function(item,text1,text2) {
        ...
        wAId = 0;
        var sid = 0;
        if (this.actor().meta[text1+'2']) wAId = this.actor().meta[text1+'2'];
        if (this.actor().meta[text2+'2']) wAId = this.actor().meta[text2+'2'];
        if (w && w.meta[text1+'2']){ wAId = w.meta[text1+'2']; mid = eval(wAId); }
        if (w && w.meta[text2+'2']){ wAId = w.meta[text2+'2']; mid = eval(wAId); }
        if (s && s.meta[text1+'2']){ wAId = s.meta[text1+'2']; sid = eval(wAId); }
        if (s && s.meta[text2+'2']){ wAId = s.meta[text2+'2']; sid = eval(wAId); }
        if (item && item.meta[text1+'2']) wAId = item.meta[text1+'2'];
        if (item && item.meta[text2+'2']) wAId = item.meta[text2+'2'];
        wAId = eval(wAId);
