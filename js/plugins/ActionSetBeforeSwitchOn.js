//
//  行動決定前及びイベント判定前スイッチオン ver1.00
//
// author yana
//

var Imported = Imported || {};
Imported['ActionSetBeforeSwitchOn'] = 1.00;
/*:
 * @plugindesc ver1.00/行動決定前やイベント判定前に条件に従ってスイッチを操作します。
 * @author Yana
 * 
	...
	Game_Troop.prototype.meetsConditions = function(page) {
		if (page.conditions.switchValid && checkFrameCount > 0 && (Graphics.frameCount % checkFrameCount === 0)){
			this.members().forEach(function(enemy){
				enemy.checkAsbsConditions('イベント');
			});
			$gameParty.members().forEach(function(actor){
				actor.checkAsbsConditions('イベント');
			});
		}
